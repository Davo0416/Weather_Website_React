...
L.OWM.LegendControl = L.Control.extend({
	...
	_redrawLegend: function() {
		...
		for (var idx in this._legendContainer) {
			...
			//item.innerHTML = '<img src="' + imgPath + '" border="0"  id = "chartImg"/>';
		}
	}
});
...
L.OWM.Current = L.Layer.extend({
		...
		if (typeof station.main != 'undefined') {
			...
			if (typeof station.main.pressure != 'undefined') {
				txt += '<div class="owm-popup-detail">'
					...
					+ ': ' + this._pressureConvert(station.main.pressure) + '&nbsp;'+ this._displayPressureUnit() +'</div>';
			}
	...
	_pressureConvert: function (pressureHpa) {
		let pressure = pressureHpa;
		switch (this.options.pressureUnit) {
			case 'psi':
				pressure = pressureHpa * 0.0145038;
				pressure = pressure.toFixed(this.options.pressureDigits);
				break;
			case 'hPa':
			default:
	...
	_displayPressureUnit: function () {
		return this.options.pressureUnit;
	},
